...
// type FooType = (x:Date) => string;
// This extracts all the d3 dependencies and allows us to inject a d3 alternative (e.g., a mock)
function d3Fns(d3g=d3) {
  return {
    axisMaker: (data: JustGraphitEntry[], x: d3.ScaleTime<number, number>, y: d3.AxisScale<number>) => {
      const bottomTickValues = tickTimeSelector(data.map(o => o.dt));

      // const foo: FooType = relative;
      const xAxis = d3g.axisBottom<Date>(x)
    ...
    grapher: (data: JustGraphitEntry[], size: Size, margin: Margin) => {
      const num = function(d: JustGraphitEntry){ return d.number};
      ...
      const y = d3g.scaleLinear<number, number>()
      ...
      const line = d3g.line<JustGraphitEntry>()
        .x(xVal)
        .y(yVal);
...
function LineGraph({height, width, chartId, graphData}: LineGraphProps): JSX.Element | null {
