...
import {GraphEntryHandler, GraphHandler, ISODatetimeType, JustGraphitEntry} from "../JustGraphitTypes";
...
function AppRouter({storage}: AppRouterProps): JSX.Element {
  ...
  // state
  const [graphObject, setGraphObject] = useState(GraphObject.fromPlainObject(initialObject));
  ...
  const addNewNumber: GraphEntryHandler = (graphName, newNumberInput) => {
    ...
    const datetime: ISODatetimeType = (new Date().toISOString()) as ISODatetimeType;
    ...
    const newGraphObject: GraphObject = new GraphObject(graphName, newGraphData);
  ...
  return (
    <HashRouter>
      ...
      <NavBar />
      <Route exact path="/" component={makePropless<GraphitProps>(Graphit as React.ComponentType, {...graphItProps})} />
      <Route exact path="/about" component={About} />
      <Route exact path="/edit" component={makePropless<EditorProps>(Editor as React.ComponentType, {...editorProps})} />
    </HashRouter>
  );
}
